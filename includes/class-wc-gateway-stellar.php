...
class WC_Gateway_Stellar extends WC_Payment_Gateway {
  ...
  public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
    if( !$sent_to_admin && $this->id === $order->payment_method && $order->has_status( 'verify-steller' ) ) {
  ...
  private function stellar_instructions( $order_id, $reciept = '' ) {
    ...
    if( $order->get_status() == 'verify-steller' ) continue; // If the order still needs verifying then display the instructions.
  ...
  public function process_payment( $order_id ) {
    ...
    // Mark as 'verify-stellar' (we're awaiting the payment)
    $order->update_status( 'verify-stellar', __( 'Awaiting Stellar payment and verification.', 'woocommerce-stellar-gateway' ) );
